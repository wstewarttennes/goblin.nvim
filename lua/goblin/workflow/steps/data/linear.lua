...
local linear = require("goblin.integrations.linear")
...
M.run = function(step, continue_workflow, update_output, input)
  ...
  local linear_res = linear.get_issues(params)
  -- Define a table
  if not linear_res then
  ...
  local i = 0
  local hashed_issues = {}
  for key, value in pairs(linear_res["data"]["issues"]["nodes"]) do
    local line_string = value["title"]
    vim.api.nvim_buf_set_lines(popup_one.bufnr, i, i, false, { line_string })

    if value["description"] then
      hashed_issues[value["title"]] = value
    end

    if i == 0 then
      if value["description"] then
        local split = splitString(value["description"], "\n")
  ...
  local function update_highlight()
    ...
    local description = hashed_issues[line_text]["description"]
