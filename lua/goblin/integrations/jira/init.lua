...
local Utils = require("goblin.integrations.jira.utils")
...
function Jira.get_issues(params)
  local param_string = "jql="
  for k, v in pairs(params) do
    param_string = param_string .. k .. "=" .. v .. "&"
  end
  local response = curl.get("https://" .. config.domain .. "/rest/api/2/search?" .. param_string, {
    headers = {
      ["Content-Type"] = "application/json",
      ["Authorization"] = "Basic " .. Utils.b64encode(config.user .. ":" .. config.token),
    },
